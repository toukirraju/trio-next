...
const ColorPalette = ({ variable = "primary", size = "md", value, onChange, }) => {
    ...
    const createLabels = () => {
        return colors.map((color, index) => {
            ...
            return (<div key={color.shade} className={`absolute transform -translate-x-1/2 -translate-y-1/2 font-semibold ${size === "sm" ? "text-[8px]" : size === "md" ? "text-[10px]" : "text-xs"}`} style={{
                    left: `${x}px`,
                    top: `${y}px`,
    ...
    const createHalfCircularText = (text, radius) => {
        ...
        return characters.map((char, index) => {
            ...
            return (<span key={index} className={`absolute transfor ${size === "sm" ? "hidden" : size === "md" ? "text-[10px]" : "text-xs"}`} style={{
    ...
    return (<div className="relative" style={{ width: `${boxSize}px`, height: `${boxSize}px` }}>
      ...
      {/* Input container */}
      <label className="absolute cursor-pointer  group rounded-full flex items-center justify-center" style={{
        ...
        <input type="color" className="w-full h-full opacity-0 cursor-pointer" value={value} onChange={onChange}/>

        {/* Circular Text */}
        <div className="relative w-full h-full">
          <span className={`absolute top-1/2 left-0 -translate-x-1/2 -translate-y-1/2 text-3xl group-hover:text-gray-100 transition-all duration-300`}>
